...
async function test() {
    console.log("Testing Firestore Connection with Where and OrderBy...");
    try {
        const q = query(collection(db, "articles"), where("category", "==", "HANDLOOM"), orderBy('createdAt', 'desc'));
        ...
        console.error("Error querying articles:", e.message);
    }
}
